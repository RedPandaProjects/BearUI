...
{
	BearUIMenuItem*Item = BearCore::bear_new< BearUIMenuItem>();
...
void BearUI::Classic::BearUIMenu::Reset()
{
	...
	for(auto b = UIItems.begin(), e = UIItems.end(); b != e; b++)
	{
		if ((*b)->Menu)
		{
			max_size_text = BearCore::bear_max((*b)->Text.size()+5, max_size_text);
		}
		else
		{
			max_size_text = BearCore::bear_max((*b)->Text.size(), max_size_text);
		}

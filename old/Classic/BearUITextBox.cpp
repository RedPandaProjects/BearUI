...
void BearUI::Classic::BearUITextBox::Reset()
{
	if (Style.test(S_richTextBox))
	{
		 ...
		 UIPlaneBackground.Rect +=BearVector4<float>(0, 0,  static_cast<float>(Font.GetHieght()),  static_cast<float>(Font.GetHieght()));
	}
	else
	{
	...
	UIText.UIText.Rect +=BearVector4<float>(2+1, 2+1, -4-2, -4-2);
...
bool BearUI::Classic::BearUITextBox::OnChar(bchar16 ch)
{
	...
	else
	{
		...
		Text.insert(UICursor.Position,1,BearEncoding::ToCurrent( ch));
		 UIText.UIText.Text = Text;
		 UIText.UIText.Reset();
		 UIText.KillSelect();
		UICursor.Position++;
	...
	if (Focused())
	{
		...
		case BearInput::KeyLeft:
			...
			else if (UIText.Selected())
			{
				Key_PressMouse = false;
				UIText.KillSelect();
				UICursor.Position =BearMath::min( UIText.UIText.SelectStart, UIText.UIText.SelectEnd);
				UICursor.Update(UIText, UIPlane);
			}
			else 
			{
		...
		case BearInput::KeyRight:
			...
			else if (UIText.Selected())
			{
				Key_PressMouse = false;
				UIText.KillSelect();
				UICursor.Position =BearMath::max(UIText.UIText.SelectStart, UIText.UIText.SelectEnd);
				UICursor.Update(UIText, UIPlane);
			}
			else
			{
...
void BearUI::Classic::BearUITextBox::Update(BearTime time)
{
...
{
	if (UIPlane.Rect.inZone(BearVector2<float>(x,y)))
	{
		if(!Key_PressMouse)
...
void BearUI::Classic::BearUITextBox::SCursor::Reset(SText & UIText)
{
	...
	UIPlane.Clip +=BearVector4<float>(-1, -1, 2, 2);
...
bint BearUI::Classic::BearUITextBox::SCursor::GetPosition(SText & UIText, BearUITexture & UIGlobalPlane,BearVector2<float> mouse_position)
{
...
{
	pos =BearMath::min(pos, UIText.Text.size());
	bsize result = 0;
...
{
	pos1 =BearMath::min(pos1, UIText.Text.size());
	pos2 =BearMath::min(pos2, UIText.Text.size());
	bsize result = 0;
...
void BearUI::Classic::BearUITextBox::SText::Copy()
{
	...
	if (selectStart > selectEnd)bear_swap(selectStart, selectEnd);
	...
	BearString::ToClipboard(&UIText.Text[selectStart], selectEnd - selectStart);
...
void BearUI::Classic::BearUITextBox::SText::Paste(bint & position)
{
	...
	auto str =BearString::GetClipboard();

...
void BearUI::Classic::BearUICursorText::Draw(BearUI * ui,BearTime time)
{
	...
	Position+=BearVector2<float>(-SizeChunk, -(Height/2));
	UITextureUp.Position = Position +BearVector2<float>(-SizeChunk, 0);
	UITextureUp.Size.y = Width;
	UITextureUp.Size.x = SizeChunk + SizeChunk + 1;
	UITextureUp.Draw(ui, time);
	UITextureMidle.Position = Position +BearVector2<float>(0, Width);
	UITextureMidle.Size.x = Width;
	UITextureMidle.Size.y = Height - Width * 2;
	UITextureMidle.Draw(ui, time);
	UITextureDown.Position = Position +BearVector2<float>(-SizeChunk, Height - Width);
